...
/*포크 사용 부모 및 자식 프로세스 생성
ls가 들어오면 execv를 사용하여 명령어 실행
-> 부모 프로세스에 이 함수를 사용하면 완전 다른 프로그램 원하는 것 실행 못하고 끝남
자식 프로세서 끝내고 부모프로세스에서 작동
wait pid로 자식이 끝날 때까지 기다렸다가 부모프로세스에서 마저 작동,
파이프 사용시 부모 자식 프로세스 복제 그 뒤로는 서로 간섭 불가
ls | grep과 같이 실행하면 ls 결과가 파이프에 들어가 grep에서 사용 파이프는 프로세스 간의 통신
